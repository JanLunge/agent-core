...
describe('end-to-end runtime smoke scenario', () => {
  it('runs normal chat through route context fake agent guarded tool session memory and progress reporting', async () => {
    ...
    expect(report.text).toContain('Completed slice: Slice 38 — Real-run audit export command');
    expect(report.text).toContain('Active slice: Slice 39 — Runtime audit export CLI fixture for Telegram spike stores');
  });
