...
describe('end-to-end runtime smoke scenario', () => {
  it('runs normal chat through route context fake agent guarded tool session memory and progress reporting', async () => {
    ...
    expect(report.text).toContain('Completed slice: Slice 47 — Runtime health/status command over local memory');
    expect(report.text).toContain('Active slice: Slice 48 — Heaper adapter migration checklist and contract gaps');
  });
