...
describe('end-to-end runtime smoke scenario', () => {
  it('runs normal chat through route context fake agent guarded tool session memory and progress reporting', async () => {
    ...
    expect(report.text).toContain('Completed slice: Slice 46 — Local audit compaction/snapshot command');
    expect(report.text).toContain('Active slice: Slice 47 — Runtime health/status command over local memory');
  });
