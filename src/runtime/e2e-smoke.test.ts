...
describe('end-to-end runtime smoke scenario', () => {
  it('runs normal chat through route context fake agent guarded tool session memory and progress reporting', async () => {
    ...
    expect(report.text).toContain('Completed slice: Slice 55 — Semantic time-range translation helper');
    expect(report.text).toContain('Active slice: Slice 56 — Pagination-safe memory scan wrapper');
  });
