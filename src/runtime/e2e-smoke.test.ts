...
describe('end-to-end runtime smoke scenario', () => {
  it('runs normal chat through route context fake agent guarded tool session memory and progress reporting', async () => {
    ...
    expect(report.text).toContain('Completed slice: Slice 52 — Runtime-status JSON output mode');
    expect(report.text).toContain('Active slice: Slice 53 — Task resume command for approval-resume tasks');
  });
