...
| --- | --- | --- | --- |
| B0-1 | Alpha smoke/spike fixtures use `FakeAgent` / scripted responders, but the beta dogfood CLI must not. | `src/cli/runtime-smoke.ts`, `src/cli/runtime-telegram-spike.ts`, `src/runtime/fake-agent.ts`, `src/cli/runtime-dogfood.ts` | Beta Slice 2: beta dogfood uses the real provider-backed responder seam; fake smoke/spike helpers are no longer exposed as product CLI commands and remain fixture/test helpers only. |
| B0-2 | `runRuntimeEvent` used to have a `defaultResponder` that returned prepared-context text if no responder was supplied. This could hide missing provider wiring. | `src/runtime/orchestrator.ts` | Beta Slice 2: fixed. `runRuntimeEvent` now requires an explicit responder and persists a blocker if runtime code omits it. |
